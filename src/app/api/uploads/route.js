...
export async function POST(req) {
  ...
  const { error } = await putObject('atelie-uploads', path, Buffer.from(await file.arrayBuffer()), { contentType: file.type });
  if (error) return Response.json({ error: 'Falha ao salvar o arquivo.' }, { status: 500 });
  const { data } = await signedUrl('atelie-uploads', path, 3600);
